...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    async function apply(ctx) {
      ...
      // 以「裁切框中心对准视口中心、等比放大铺满」为目标，全部用 calc/max + 无单位系数表达。
      const buildImageCss = (url, natural) => {
        ...
        const c = state.crop
        const A = natural.w * c.fw // 裁切框宽(px)
        const B = natural.h * c.fh // 裁切框高(px)
        const X = natural.w * c.fx // 裁切框左(px)
        const Y = natural.h * c.fy // 裁切框顶(px)
        ...
        const ry1 = f(Y / A)
        const ry2 = f(Y / B)
        const Sx = 'max(100vw, calc(100vh * ' + rA + '))'
        const Sy = 'max(calc(100vw * ' + rB + '), 100vh)'
        const Px = 'calc(50vw - max(100vw * ' + rx1 + ', 100vh * ' + rx2 + ') - max(100vw, calc(100vh * ' + rA + ')) * 0.5)'
        const Py = 'calc(50vh - max(100vw * ' + ry1 + ', 100vh * ' + ry2 + ') - max(calc(100vw * ' + rB + '), 100vh) * 0.5)'
