    ...
    async function apply(ctx) {
      ...
      try {
        ...
        if (res && res.ok === true && res.state && typeof res.state === 'object') {
          ...
          if (state.mode === 'local' && state.selected) state.localUrl = '/dsh-bg/api/media?v=' + Date.now()
      ...
      const UI_CSS = '.dshbg-page{display:flex;flex-direction:column;gap:12px;padding:2px 0 8px;max-width:560px}' +
        ...
        // —— 背景视频层：位于压暗遮罩(-1)之下、应用表面之上，等比 cover 铺满 ——
      ...
      const styleVideo = (v, blur) => {
        v.style.filter = blur > 0 ? 'blur(' + blur + 'px)' : ''
        v.style.transform = blur > 0 ? 'scale(1.08)' : ''
      }

      const ensureVideo = (src, blur) => {
        if (videoEl !== null && videoEl.dataset.src === src) {
          styleVideo(videoEl, blur)
          if (videoEl.paused) {
      ...
      const applyBackground = () => {
        ...
        if (!src) return
        // 背景清晰度：越高表面越透明、媒体越清楚；内容卡片表面略不透明保证可读
        ...
        if (state.kind === 'video') {
          // 视频：<video> 层等比 cover 铺满，此处只叠加压暗遮罩（不做裁切）
          ensureVideo(src, blur)
          const css = 'body::before{content:"";position:fixed;inset:0;z-index:-1;pointer-events:none;' +
            'background-image:' + scrim + ';background-repeat:no-repeat;background-size:auto;' +
        ...
        function BackgroundSettings() {
          ...
          const loadDir = async (dir) => {
            ...
            try {
              ...
              const list = res && res.ok === true && Array.isArray(res.files)
                ? res.files.slice().sort((a, b) =>
          ...
          return h('div', { className: 'dshbg-page' }, [
            ...
            h('div', { className: 'dshbg-row' }, [
              h('button', {
                ...
                onClick: () => {
                  state.urlDraft = ''
                  apply({ enabled: false, mode: 'url', kind: 'image', url: '', dirDraft: '', dir: '', files: [], selected: '', localUrl: '', clarity: 60, dim: 0.2, blur: 0, crop: { fx: 0, fy: 0, fw: 1, fh: 1 }, error: '' })
                },
              }, '恢复默认'),
            ]),
